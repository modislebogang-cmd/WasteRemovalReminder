...
export async function saveSelectedBranch(staffNumber, selectedBranchId) { if (db && staffNumber && selectedBranchId) return setDoc(doc(db, "staff", staffNumber), { selectedBranchId }, { merge: true }); }
...
export async function saveBranchMemberProfile(branchId, userId, profile) { return setDoc(doc(db, "branches", branchId, "members", userId), { ...profile, userId, updatedAt: serverTimestamp() }, { merge: true }); }
